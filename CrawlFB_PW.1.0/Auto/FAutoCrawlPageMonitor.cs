...
namespace CrawlFB_PW._1._0.Auto
{
    public partial class FAutoCrawlPageMonitor : Form
    {
        ...
        //  List<PageInfoViewModel> _pageList;
        ...
        private void btn_StartAuto_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            _autoService.SetConfig(startTime, delayStr);
            Libary.Instance.LogForm(module, $"⚙ SetCongif: thời gian bắt đầu: {startTime} --- delay time: {delayStr}");
            // 🔥 EVENT PROGRESS
            _autoService.OnProgress += (pageId, status, newPosts, savedPosts) =>
            {
                if (!_monitorDict.TryGetValue(pageId, out var vm))
                    return;

                var name = vm?.PageName ?? pageId;
                Libary.Instance.LogForm("UI",
        $"📊 PROGRESS | Page={name} | Status={status} | New={newPosts} | Saved={savedPosts}");
                this.Invoke(new Action(() =>
                {
                    ...
                    UpdateResult(pageId, newPosts, savedPosts);

                    // =========================
                    // 🔥 COUNT TOTAL
                    // =========================
                    if (newPosts > 0) _totalNew += newPosts;
                    if (savedPosts > 0) _totalSaved += savedPosts;

                    // =========================
                    // 🔥 FIX TAB COUNT (HashSet)
                    // =========================
                    lock (_runningLock)
                    {
                        if (status == AutoService.PageStatus.Running)
                        {
                            _runningPages.Add(pageId); // add nhưng không bao giờ trùng
                        }
                        else if (status == AutoService.PageStatus.StopScroll
                              || status == AutoService.PageStatus.Idle)
                        {
                            _runningPages.Remove(pageId);
                        }

                        _runningTabs = _runningPages.Count;
                    }
                    ...
                    if (status == AutoService.PageStatus.StopScroll && vm != null)
                    {
                        ...
                        {
                            vm.DelayExtra += 600;
                            Libary.Instance.LogForm(module, $"⚠ Page {name} không có bài → +10 phút delay");
                        }
                        else
                        {
                            vm.DelayExtra = 0;
                        ...
                        Libary.Instance.LogForm(module, $"⏹ Xong Page: {name} | +{newPosts} post");

                        if (_isAutoSave)
                            Libary.Instance.LogForm(module, $"💾 Đã lưu {savedPosts} post");
                        else
                            Libary.Instance.LogForm(module, $"🧪 TEST MODE: {newPosts} post");

                        Libary.Instance.LogForm(module, $"😴 Delay {vm.Countdown}s");
                    }

                    // =========================
                    // 🔥 RESTING LOG
                    // =========================
                    if (status == AutoService.PageStatus.Resting)
                    {
                        Libary.Instance.LogForm(module, $"💤 Page {name} đang nghỉ...");
                    }
                    ...
                    Libary.Instance.LogForm(module,
                        $"📊 Tabs: {_runningTabs}/{_selectedProfiles.Count * 3}");
                }));

                // =========================
                // 🔥 RUNNING LOG (ngoài invoke cho nhẹ UI)
                // =========================
                if (status == AutoService.PageStatus.Running)
                {
                    Libary.Instance.LogForm(module, $"▶ Đang chạy Page: {name}");
                }
            };
            //===
            _autoService.OnNewPosts += (pageId, posts, shares) =>
            {
                PageMonitorViewModel vmMonitor;
                _monitorDict.TryGetValue(pageId, out vmMonitor);
                var name = vmMonitor != null ? vmMonitor.PageName : pageId;
        ...
        private void btn_StopAuto_ItemClick(object sender, ItemClickEventArgs e)
        {
            // 🔥 stop service
            _autoService?.Stop();
            _autoService = null;
            ...
            if (_isAutoSave)
            {
                foreach (var pageInfo in _monitorList)
                {
                    ...
                    {
                        // chỉ update nếu lớn hơn (tránh update ngược)
